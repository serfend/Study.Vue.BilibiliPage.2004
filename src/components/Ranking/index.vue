<template>
  <div class="ranking-list-wrap">
    <ul v-if="!loading" class="rank-list" :class="{ bangumi: type == 2 }">
      <li class="rank-item" v-for="(item, index) in rankAll.list" :key="`rankall_list_${index}`">
...
<script>
import biliLoading from "@/components/Loading";
import { count2 } from "@/utils";
export default {
  name: "Ranking",
  props: {
    rankAll: {
